...
const [p1, p2] = h.game.board.columns[0].picks;
ok(p1.round === 1 && p1.slot === 1 && p2.round === 2 && p2.slot === 1, `picks numbered draft-style (${p1.round}.${p1.slot}, ${p2.round}.${p2.slot})`);
host.act({ type: "draft/setArt", pickId: "nope", art: { emoji: "🪓" } });
ok((await host.error()).includes("Unknown pick"), "art for an unknown pick refused");
